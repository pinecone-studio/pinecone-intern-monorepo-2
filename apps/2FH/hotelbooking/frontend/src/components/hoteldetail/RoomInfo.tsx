...
const infoIcons: Record<string, React.ReactNode> = {
  private_bathroom: <ShowerHead className="w-4 h-4" />,
  shared_bathroom: <Toilet className="w-4 h-4" />,
  air_conditioner: <AirVent className="w-4 h-4" />,
  free_bottle_water: <Droplet className="w-4 h-4" />,
    ...
    <div className="flex flex-col gap-y-4">
      ...
      <div className="grid grid-cols-3 grid-rows-2 gap-4 px-10">
        {data?.getRooms?.map((rooms) => (
          <div key={rooms.id} className="flex flex-col gap-y-4">
            <div className="w-[349px] h-[216px] relative">
              <Image src={rooms.imageURL[0] ?? '/fallback.jpg'} alt="roomImage" fill className="object-cover rounded-lg" />
            </div>
              ...
              <div className="flex flex-col gap-y-3">
                {rooms.roomInformation.slice(0, 6).map((info, index) => (
                  <div key={index} className="flex items-center gap-x-2 text-sm">
                    {infoIcons[info] ?? <Wifi className="w-4 h-4" />}
