...
describe('createBooking resolver', () => {
  ...
  describe('successful execution', () => {
    it('should create booking successfully with valid input', async () => {
      ...
      expect(mockBookingModel.create).toHaveBeenCalledWith(expect.objectContaining({ ...validInput, status: 'booked' }));
  ...
  describe('validation errors', () => {
    it('should throw error when check-out date is before check-in date', async () => {
      ...
      await expect(createBookingResolver(mockParent, { input: invalidInput }, mockContext, mockInfo)).rejects.toThrow(GraphQLError);
    });
    ...
    it('should throw error when check-in date is in the past', async () => {
      ...
      await expect(createBookingResolver(mockParent, { input: pastInput }, mockContext, mockInfo)).rejects.toThrow(GraphQLError);
    });
  ...
  describe('database errors', () => {
    it('should handle generic errors and convert to GraphQLError', async () => {
      ...
      await expect(createBookingResolver(mockParent, { input: validInput }, mockContext, mockInfo)).rejects.toThrow(GraphQLError);
    });
