...
import { getBookingsByCheckOutDate } from '../../../../src/resolvers/queries/booking';
...
const getBookingsByCheckOutDateResolver = getBookingsByCheckOutDate;
...
describe('getBookingsByCheckOutDate resolver', () => {
  ...
  describe('successful execution', () => {
    it('should fetch and return transformed bookings for valid date', async () => {
      ...
      const mockBookings = [
        { id: '1', checkOutDate: new Date('2024-01-15T10:00:00Z') },
        { id: '2', checkOutDate: new Date('2024-01-15T15:00:00Z') }
      ];
      const mockTransformedBookings = [
        { id: '1', checkOutDate: '2024-01-15T10:00:00.000Z' },
        { id: '2', checkOutDate: '2024-01-15T15:00:00.000Z' }
      ];
      ...
      // Act
      const result = await getBookingsByCheckOutDateResolver(mockParent, { checkOutDate }, mockContext, mockInfo);
  ...
  describe('date validation', () => {
    it('should throw error for invalid date format', async () => {
      ...
      // Act & Assert
      await expect(getBookingsByCheckOutDateResolver(mockParent, { checkOutDate: invalidDate }, mockContext, mockInfo))
        .rejects.toThrow(GraphQLError);

      try {
        await getBookingsByCheckOutDateResolver(mockParent, { checkOutDate: invalidDate }, mockContext, mockInfo);
      } catch (error) {
        expect(error).toBeInstanceOf(GraphQLError);
        expect((error as GraphQLError).message).toBe('Invalid check-out date format');
    ...
    it('should throw error for empty date string', async () => {
      ...
      // Act & Assert
      await expect(getBookingsByCheckOutDateResolver(mockParent, { checkOutDate: emptyDate }, mockContext, mockInfo))
        .rejects.toThrow(GraphQLError);
    });
  });
  ...
  describe('error handling', () => {
    it('should throw GraphQLError when database query fails', async () => {
      ...
      // Act & Assert
      await expect(getBookingsByCheckOutDateResolver(mockParent, { checkOutDate }, mockContext, mockInfo))
        .rejects.toThrow(GraphQLError);

      try {
        await getBookingsByCheckOutDateResolver(mockParent, { checkOutDate }, mockContext, mockInfo);
      } catch (error) {
        expect(error).toBeInstanceOf(GraphQLError);
        expect((error as GraphQLError).message).toBe('Failed to fetch bookings by check-out date');
        expect((error as GraphQLError).extensions?.code).toBe('CHECKOUT_DATE_BOOKINGS_FETCH_FAILED');
      }
    });
