...
describe('Posts Component - Props', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  describe('Component Props', () => {
    it('should pass userName prop to the query', () => {
      mockUseGetUserByUsernameQuery.mockReturnValue({
        data: { getUserByUsername: mockUserData },
        loading: false
      });
      render(<Posts userName="testuser123" />);
      expect(mockUseGetUserByUsernameQuery).toHaveBeenCalledWith({
        variables: { userName: 'testuser123' }
      });
    });
  });
});
