...
describe('useSearchLogic - GraphQL Callbacks Part 3', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers();
  ...
  it('should call onError callback for removeFromSearchHistory', () => {
    ...
    if (capturedOnError) {
      capturedOnError(new Error('Remove from history failed'));
    }
    expect(consoleSpy).toHaveBeenCalledWith('Remove from search history error:', expect.any(Error));
