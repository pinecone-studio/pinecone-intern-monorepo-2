...
describe('useSearchLogic - GraphQL Callbacks Part 6', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers();
  ...
  it('should call onError callback for addToSearchHistory', () => {
    ...
    if (capturedOnError) {
      capturedOnError(new Error('Add to history failed'));
    }
    expect(consoleSpy).toHaveBeenCalledWith('Add to search history error:', expect.any(Error));
