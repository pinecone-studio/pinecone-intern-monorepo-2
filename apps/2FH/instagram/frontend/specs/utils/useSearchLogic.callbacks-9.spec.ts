...
describe('useSearchLogic - GraphQL Callbacks Part 9', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers();
  ...
  it('should not refetch when clearSearchHistory fails', () => {
    ...
    if (capturedOnCompleted) {
      capturedOnCompleted({ clearSearchHistory: { success: false } });
    }
    expect(mockRefetchSearchHistory).not.toHaveBeenCalled();
  });
});