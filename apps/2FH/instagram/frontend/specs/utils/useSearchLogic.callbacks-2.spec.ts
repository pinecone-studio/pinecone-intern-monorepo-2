...
describe('useSearchLogic - GraphQL Callbacks Part 2', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers();  
    ...
    mockUseAddToSearchHistoryMutation.mockReturnValue([
      ...
      { data: undefined, loading: false, error: undefined, called: false, client: {} as any, reset: jest.fn()}
    ]);
  ...
  it('should call onCompleted callback for removeFromSearchHistory', () => {
    let capturedOnCompleted: any;
    mockUseRemoveFromSearchHistoryMutation.mockImplementation((options) => {
    ...
    if (capturedOnCompleted) {
      capturedOnCompleted({ removeFromSearchHistory: { success: true } });
    }
    expect(mockRefetchSearchHistory).toHaveBeenCalled();
  });
  it('should call onError callback for clearSearchHistory', () => {
    const consoleSpy = jest.spyOn(console, 'error').mockImplementation();
    let capturedOnError: any;
    mockUseClearSearchHistoryMutation.mockImplementation((options) => {
    ...
    renderHook(() => useSearchLogic('current-user', true));
    if (capturedOnError) {
      capturedOnError(new Error('Clear history failed'));
    }
    expect(consoleSpy).toHaveBeenCalledWith('Clear search history error:', expect.any(Error));
