...
describe('useSearchLogic - Interactions Part 2', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers();  
  ...
  it('should handle remove from history', () => {
    const { result } = renderHook(() => useSearchLogic('current-user', true));
    act(() => {
      result.current.handleRemoveFromHistory('user-3');
    });
    expect(mockRemoveFromSearchHistory).toHaveBeenCalledWith({
      variables: { searchedUserId: 'user-3' }
    });
  });
});