...
describe('useSearchLogic - GraphQL Callbacks Part 1', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers();
  ...
  it('should call onCompleted callback for clearSearchHistory', () => {
    let capturedOnCompleted: any;
    ...
    renderHook(() => useSearchLogic('current-user', true));
    if (capturedOnCompleted) {
      capturedOnCompleted({ clearSearchHistory: { success: true } });
    }
    expect(mockRefetchSearchHistory).toHaveBeenCalled();
  });
});