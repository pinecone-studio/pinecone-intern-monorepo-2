...
const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, _files: File[], setFiles: (_files: File[]) => void) => {
  const newFiles = Array.from(e.target.files || []).filter((f): f is File => f.type.startsWith('image/'));
  if (newFiles.length) setFiles([..._files, ...newFiles]);
};

const NavigationButtons = ({ files, currentIndex, onPrev, onNext }: { files: File[]; currentIndex: number; onPrev: () => void; onNext: () => void }) => {
  if (files.length <= 1) return null;

  return (
    <>
      <button onClick={onPrev} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/60 text-white w-8 h-8 rounded-full">
        ‹
      </button>
      <button onClick={onNext} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/60 text-white w-8 h-8 rounded-full">
        ›
      </button>
...
const ThumbnailTray = ({ files, setFiles, addMoreInputRef }: { files: File[]; setFiles: (_files: File[]) => void; addMoreInputRef: React.RefObject<HTMLInputElement | null> }) => (
...
const CropStageComponent = ({ files, setFiles, stage, Stages }: CropStageProps) => {
  ...
  const showEditControls = stage === Stages[1];

  return (
    <div className="w-full flex justify-center relative">
      <div className="w-full max-w-2xl h-full relative">
        <input ref={addMoreInputRef} type="file" accept="image/*" multiple className="hidden" onChange={handleFileChangeClick} />

        {files.length > 0 && (
          <div className="w-full max-w-4xl max-h-[80vh] flex justify-center items-center overflow-hidden">
            <img src={URL.createObjectURL(files[currentIndex])} alt={`Preview ${currentIndex + 1}`} className="w-110 aspect-[4/5] object-contain" />

            <NavigationButtons files={files} currentIndex={currentIndex} onPrev={handlePrevImage} onNext={handleNextImage} />
            ...
            {showThumbTray && showEditControls && <ThumbnailTray files={files} setFiles={setFiles} addMoreInputRef={addMoreInputRef} />}
          </div>
        )}
      </div>
    </div>
...
export const CropStage = CropStageComponent;
