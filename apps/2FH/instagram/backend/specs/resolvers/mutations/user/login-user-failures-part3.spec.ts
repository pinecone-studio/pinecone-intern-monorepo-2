...
describe('loginUser - Authentication Failures', () => {
  ...
  it('should handle missing password field in user document', async () => {
    ...
    // Make the final populate call resolve to the user document without password
    mockQuery.populate
      .mockReturnValueOnce(mockQuery) // First populate call
      .mockReturnValueOnce(mockQuery) // Second populate call  
      .mockReturnValueOnce(mockQuery) // Third populate call
      .mockResolvedValueOnce(userWithoutPassword as never); // Fourth populate call resolves
