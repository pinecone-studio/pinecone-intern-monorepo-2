...
const handleError = (error: unknown): never => {
  ...
  throw new GraphQLError('Failed to view story: ' + (error instanceof Error ? error.message : 'Unknown error'));
};

export const viewStory = async (_: unknown, { _id }: { _id: string }, context: { userId: string }) => {
  try {
