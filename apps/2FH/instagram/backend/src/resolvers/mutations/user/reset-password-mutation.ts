...
const handleResetPasswordProcess = async (user: UserDocument, email: string, otp: string, newPassword: string): Promise<void> => {
...
export const resetPassword = async (_parent: unknown, { input }: { input: ResetPasswordInput }): Promise<boolean> => {
  try {
