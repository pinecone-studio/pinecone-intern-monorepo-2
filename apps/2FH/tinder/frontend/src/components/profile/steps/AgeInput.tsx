    ...
    <div className="flex flex-col items-center  bg-white h-full">
      <div className="w-full max-w-md text-center">
        <div className="text-left ">
          ...
          <p className="text-xs text-gray-500">Your date of birth is used to calculate your age.</p>
        </div>

        <div className="flex my-5 justify-between w-full">
          <button onClick={prevStep} className="py-1 px-3 border border-gray-300 rounded-full font-semibold text-gray-700 hover:bg-gray-50 transition-all duration-200">
            Back
          </button>
          <button
            onClick={nextStep}
            disabled={!canProceed}
            className={`px-3 py-1 rounded-full font-semibold text-white transition-all duration-200 ${canProceed ? 'bg-[#E11D48E5] hover:opacity-80' : 'bg-gray-300 cursor-not-allowed'}`}
          >
            Next
          </button>
        </div>
