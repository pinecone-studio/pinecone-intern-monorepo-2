...
export const useTinderSwipe = (currentUserId: string): SwipeState & SwipeActions => {
  ...
  const sendSwipeToBackend = useCallback(async (profileId: string, direction: SwipeDirection) => {
    ...
    try {
      const action = direction === 'right' ? SwipeAction.Like : SwipeAction.Dislike;
  ...
  const handleSwipe = useCallback((direction: SwipeDirection) => {
    ...
    sendSwipeToBackend(currentProfile.userId, direction);
  ...
  const handleSuperLike = useCallback(() => handleSwipe('right'), [handleSwipe]);
