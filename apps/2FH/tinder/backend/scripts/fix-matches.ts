...
const validateMutualMatch = async (userId1: string, userId2: string) => {
  try {
...
const cleanupUserMatches = async (userId: string) => {
  try {
    ...
    for (const matchId of profile.matches) {
      const isValid = await validateMutualMatch(userId, matchId.toString());
      if (isValid) {
...
const cleanupAllMatches = async () => {
  try {
    ...
    for (const profile of profiles) {
      await cleanupUserMatches(profile.userId.toString());
    }
