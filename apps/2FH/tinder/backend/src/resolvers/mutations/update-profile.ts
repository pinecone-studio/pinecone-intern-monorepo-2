...
export const updateProfile: MutationResolvers['updateProfile'] = async (_, { input }) => {
  try {
    ...
    const updatedProfile = await Profile.findOneAndUpdate({ userId: input.userId }, { $set: updateData }, { new: true });
