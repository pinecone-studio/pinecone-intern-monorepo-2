...
const root = './apps/1CD/concert-ticket-booking/frontend/src/app';
...
export const getAllPages = () => {
  ...
  pages = pages.map((page) =>
    page
      .split('/')
      .filter((pagePath) => !pagePath.includes('('))
      .join('/')
  );

  fs.writeFileSync(path.join(...'apps/1CD/concert-ticket-booking/frontend'.split('/'), 'cypress', 'utils', 'all-pages.json'), JSON.stringify(pages));
