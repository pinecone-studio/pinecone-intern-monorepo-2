...
const OrderInfo = () => {
  ...
  return (
    ...
    return (<Card
      className="bg-[#131313] border-none  px-8 pt-8 pb-6 mb-8"
      key={order?._id}
      data-cy={`order-card-${order?._id}`}
    >
      <div className="mb-4 text-white lg:flex lg:items-center lg:justify-between sm:max-w-flex sm:max-w-flex-col ">
        ...
        {order?.status === 'pending' && (
          <div data-cy={`order-status-pending-${order?._id}`}>
            ...
            <span className="rounded-full bg-black py-2 px-[10px] border-[1px] border-[#27272A] text-xs font-semibold">
              Цуцлах хүсэлт илгээсэн
            </span>
          </div>
        )}
        ...
        {order?.status === 'available' && (
          <>
            <Button
              ...
              data-cy={`cancel-button-${order?._id}`}
            >
              Цуцлах
            </Button>
            <DialogComponent
      ...
      {order?.ticketType?.map((ticket, index) => {
        ...
        const discountedPrice = Number(
          (Number(ticket.unitPrice) * (100 - discount)) / 100
        );
        const soldQuantity = Number(ticket.soldQuantity);
        return (
          <div
          ...
          >
              ...
              <span
                className={`${
                  index == 0
                    ...
                    ? 'text-[#C772C4]'
                    : 'text-white'
                } flex gap-2 items-center font-bold text-sm`}
                data-cy={`ticket-zone-${index}}`}
              >
                <div
                  className={`${
                    index == 0
            ...
            <span
              ...
              data-cy={`ticket-price-${index}`}
            >
              <span className="text-base font-normal text-muted-foreground">
                {toMNT(discountedPrice)}×{ticket.soldQuantity}
              </span>
              <span>
                {toMNT(discountedPrice * soldQuantity)}
              </span>
            </span>
          </div>
        );
