...
import { addToCarts } from "../../../../src/resolvers/mutations/order/addto-cart";
...
beforeEach(()=>{
 ...
 it('ordering a ticket', async()=>{
...
).mockResolvedValueOnce({userId, ...input});

    const result=await addToCarts!({}, {input}, {userId}, {}as GraphQLResolveInfo);
    expect(Ticket.findById as jest.Mock).toHaveBeenCalledWith(input.ticketId);
    ...
    expect(result).toEqual({userId:userId, ...input});
   });
   it('throw a error if seats are full', async()=>
{
    const mockOrder={
        eventId:input.eventId,
        ticketId:input.ticketId,
