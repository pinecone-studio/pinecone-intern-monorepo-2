import { login } from '../../../../src/resolvers/mutations/auth-mutations/login';
...
describe('login', () => {
  it('should throw an error if user is not found', async () => {
    ...
    try {
      await login!({}, { email: 'test@example.com', password: 'password123' }, { userId: null }, {} as GraphQLResolveInfo);
    } catch (error) {
  ...
  it('should throw an error if password does not match', async () => {
    ...
    try {
      await login!({}, { email: 'test@example.com', password: 'wrongPassword' }, { userId: null }, {} as GraphQLResolveInfo);
    } catch (error) {
  ...
  it('should login successfully', async () => {
    ...
    const response = await login!({}, { email: 'test@example.com', password: 'password123' }, { userId: null }, {} as GraphQLResolveInfo);
