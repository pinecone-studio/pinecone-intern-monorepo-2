...
describe('verifying the otp', () => {
  ...
  it('should throw error when input is empty', async () => {
    await expect(verifyOtp!({}, { input: { email: '', otp: '0000' } }, {}, mockInfo)).rejects.toThrow(GraphQLError);
    await expect(verifyOtp!({}, { input: { email: '', otp: '0000'} }, {}, mockInfo)).rejects.toThrow('Email or Otp are required');
  });
