...
export const ChooseHourlyOrDaily = ({ formik, data }: { formik: FormikProps<RequestFormValues>; data: CreateRequestQuery | undefined }) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      {formik.values.requestType !== 'remote' && <RequestTypeSelector typeRequest={typeRequest} setType={setType} />}
      {(formik.values.requestType === 'remote' || typeRequest) && <PickDate formik={formik} />}
...
const RequestTypeSelector = ({ typeRequest, setType }: { typeRequest: string; setType: React.Dispatch<React.SetStateAction<string>> }) => (
  <>
    <div className="text-[#000000] text-sm">
      Төрөл{typeRequest && <></>}<span className="text-[#EF4444]">*</span>
    </div>
...
const PickDate = ({ formik }: { formik: FormikProps<RequestFormValues> }) => {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="text-[#000000] text-sm">
        Чөлөө авах өдөр{requestDate && <></>}<span className="text-[#EF4444]">*</span>
      </div>
    ...
    <div className="w-full grid grid-cols-2 gap-6">
      <div>
        <div className="text-[#000000] text-sm mb-2">
          Эхлэх цаг{validation && <></>}<span className="text-[#EF4444]">*</span>
        </div>
        <ComboboxDemo
          placeholder="00:00"
          haveSearch={false}
      ...
      <div>
        <div className="text-[#000000] text-sm mb-2">
          Дуусах цаг{formik.values.endTime && <></>}<span className="text-[#EF4444]">*</span>
        </div>
        <ComboboxDemo
          placeholder="00:00"
          haveSearch={false}
...
const SupervisorSelector = ({ formik, options }: { formik: FormikProps<RequestFormValues>; options: { label: string; value: string }[] }) => (
  <div>
    ...
    {options.length > 0 && (
      <ComboboxDemo
        options={options}
        onChange={(value) => formik.setFieldValue('supervisorEmail', value)}
      />
    )}
  </div>
);
