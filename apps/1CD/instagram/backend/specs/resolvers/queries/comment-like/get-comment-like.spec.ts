...
import { getCommentLike } from '../../../../src/resolvers/queries';
...
describe('get  post likes', () => {
  it('should get  post like', async () => {
    const response = await getCommentLike!({}, { commentId: 'post1' }, { userId: '1' }, {} as GraphQLResolveInfo);
  ...
  it('should throw an error when userId is not provided', async () => {
    await expect(getCommentLike!({}, { commentId: 'post1' }, { userId: null }, {} as GraphQLResolveInfo)).rejects.toThrow('Unauthorized');
  });
});
