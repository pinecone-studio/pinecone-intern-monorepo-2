...
export type notification = {
  ...
  otherUserId: { userName: string; profileImg?: string | null; __typeName?: string };
  createdAt: Date;
  isViewed: boolean;
...
const Notification = () => {
  ...
  if (!notifyData.getNotificationsByLoggedUser) return <NoNotification data-cy="noNotificationComp" />;
  ...
  const thisWeekNotify = sortedNotify.filter((oneNotify) => isThisWeek(new Date(oneNotify.createdAt)) && !isYesterday(new Date(oneNotify.createdAt)));
  const earlierNotify = sortedNotify.filter((oneNotify) => isThisYear(new Date(oneNotify.createdAt)) && !isThisWeek(new Date(oneNotify.createdAt)));
