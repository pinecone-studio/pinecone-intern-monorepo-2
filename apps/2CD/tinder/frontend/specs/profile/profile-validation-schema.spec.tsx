...
describe("profileValidationSchema", () => {
  ...
  it("should fail when required fields are missing", async () => {
    ...
    try {
      await profileValidationSchema.validate(invalidProfile, { abortEarly: false });
    } catch (err: any) {
      const errorMessages = err.errors.join(" ");
      expect(errorMessages).toMatch(/User ID is required/);
      expect(errorMessages).toMatch(/Looking For is required/);
    }
  });

  it("should fail when age is under 18", async () => {
    const profile = { ...validProfile, age: 17 };
    await expect(profileValidationSchema.validate(profile)).rejects.toThrow("You must be at least 18");
  });
  ...
  it("should fail with no interests", async () => {
    ...
    await expect(profileValidationSchema.validate(profile)).rejects.toThrow("Select at least one interest");
  });

  it("should fail with no images", async () => {
    const profile = { ...validProfile, images: [] };
    await expect(profileValidationSchema.validate(profile)).rejects.toThrow("At least one image is required");
  });
