            ...
            <div className="p-4">
              ...
              {hotel.averageRating && (
                <div className="flex items-center mt-4">
                  <span className="bg-blue-600 text-white text-sm px-2 py-1 rounded-md font-semibold">{hotel.averageRating}</span>
                  <span className="ml-2 text-sm text-gray-700 font-medium">{hotel.averageRating >= 8 ? 'Excellent' : hotel.averageRating >= 6 ? 'Good' : 'Average'}</span>
                </div>
              )}
            </div>
          </div>
