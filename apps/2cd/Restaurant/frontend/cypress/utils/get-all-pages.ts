...
const root = './apps/2cd/Restaurant/frontend/src/app';
...
const searchDepth = (folderPath: string) => {
  ...
  if (rootFolders.includes('page.tsx')) {
    pages.push(`/apps/2cd/Restaurant/frontend${folderPath}`);
  }
...
export const getAllPages = () => {
  ...
  fs.writeFileSync(path.join(...'apps/2cd/Restaurant/frontend'.split('/'), 'cypress', 'utils', 'all-pages.json'), JSON.stringify(pages));
